          ...
          <div key={idx} className="experience-item grid grid-cols-1 md:grid-cols-[130px_1fr] gap-x-6 gap-y-1">
            ...
            <div className="space-y-1.5">
              <div className="flex flex-col gap-0.5">
                <h3 className="text-[16px] font-black tracking-tighter leading-none uppercase">
                  {exp.role} 
                </h3>
                <div className="flex items-center gap-3">
                  {exp.url ? (
                    <a 
                      ...
                      className="text-accent font-black text-[11px] hover:underline inline-flex items-center gap-1"
                    >
                      {exp.company}
                      <ExternalLink className="w-2.5 h-2.5" />
                    </a>
                  ) : (
                    <span className="text-black font-black text-[11px]">{exp.company}</span>
                  )}
                  {exp.tech && (
                    <div className="flex items-center gap-1 px-1.5 py-0.5 bg-black text-white text-[8px] font-black uppercase tracking-widest">
                      <Zap className="w-2 h-2 fill-accent text-accent" /> {exp.tech}
                    </div>
                  )}
                </div>
              </div>

              <ul className="space-y-1 text-[11px] leading-[1.3] font-medium text-foreground/90 border-l border-black pl-4">
                {exp.achievements.map((item, i) => (
                  <li key={i} className="relative before:content-[''] before:absolute before:-left-[18.5px] before:top-[6px] before:w-1 before:h-1 before:bg-accent">
                    {item}
                  </li>
                ))}
              </ul>
